        ...
        foreach (int id in PrefixGroupCatalog.PrefixIdsFor(group.Group, slot.IsCalamity, slot.Item.Id, _service.PrefixRules))
        {
            ...
            if (id >= CalamityIds.PrefixIdBase)
            {
                ...
                string rogueName = rogueEntry == null ? $"#{id}"
                    : Terrakeep.Core.Data.LocalizedContent.Pick(rogueEntry.Es, rogueEntry.En, LocalizationService.Instance.Language);
            ...
            string name = entry == null ? $"#{id}"
                : Terrakeep.Core.Data.LocalizedContent.Pick(entry.Es, entry.En, LocalizationService.Instance.Language);
